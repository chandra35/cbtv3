...
@section('page-title', 'Edit Question Group')
@section('page-subtitle', $questionGroup->name)

@section('content')
<div class="max-w-4xl mx-auto">
    <div class="bg-white rounded-lg shadow-sm border border-gray-200">
        <form action="{{ route('admin.question-groups.update', $questionGroup) }}" method="POST" class="p-6">
            @csrf
            @method('PUT')
            ...
            <div class="mb-8">
                ...
                <div class="mb-6">
                    ...
                    <textarea id="description" name="description" 
                        ...
                        rows="4">{{ old('description', $questionGroup->description) }}</textarea>
                    @error('description')
                        <p class="mt-1 text-sm text-red-500">{{ $message }}</p>
                    @enderror
                </div>
                ...
                <div class="space-y-4">
                    ...
                    <div class="flex items-center">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" name="randomize_questions" value="1"
                                {{ old('randomize_questions', $questionGroup->randomize_questions) ? 'checked' : '' }}
                    ...
                    <div class="flex items-center">
                        <label class="flex items-center cursor-pointer">
                            <input type="checkbox" name="randomize_options" value="1"
                                {{ old('randomize_options', $questionGroup->randomize_options) ? 'checked' : '' }}
                                class="h-4 w-4 text-blue-500 border-gray-300 rounded focus:ring-2 focus:ring-blue-500">
                            <span class="ml-3">
                    ...
                    <div class="mt-6 pt-6 border-t border-gray-200">
                        ...
                        <input type="number" id="show_questions_per_page" name="show_questions_per_page" 
                            value="{{ old('show_questions_per_page', $questionGroup->show_questions_per_page) }}"
            ...
            <div class="bg-gray-50 rounded-lg p-4 mb-8">
                <p class="text-sm font-semibold text-gray-700 mb-3">Group Statistics</p>
                <div class="grid grid-cols-3 gap-4">
                    <div>
                        <p class="text-xs text-gray-600">Total Questions</p>
                        <p class="text-2xl font-bold text-gray-900">{{ $questionGroup->questions->count() }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-600">Total Points</p>
                        <p class="text-2xl font-bold text-gray-900">{{ $questionGroup->questions->sum('point') }}</p>
                    </div>
                    <div>
                        <p class="text-xs text-gray-600">Created</p>
                        <p class="text-sm font-medium text-gray-900">{{ $questionGroup->created_at->format('d M Y') }}</p>
                    </div>
                </div>
            </div>
            ...
            <div class="flex gap-4">
                ...
                <a href="{{ route('admin.exams.show', $questionGroup->exam) }}" class="px-6 py-2 bg-gray-300 text-gray-800 rounded-lg hover:bg-gray-400 transition duration-200 font-medium">
